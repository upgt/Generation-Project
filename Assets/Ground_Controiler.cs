...
namespace Assets.Texture
{
    public class Ground_Controiler : MonoBehaviour
    {
        ...
        float[,] heights;
        public void StartGroundControl(TerrainGenerator terrainGenerator)
        {
        ...
        public void AddTexture(List<GroundInfo> texPrototypes, Calculated funk, float[,] mask)
        {
            ...
            int protTwo = GetNotRepeatRand(texPrototypes.Count - 1, prot);
        ...
        private int GetNotRepeatRand(int max, int notRepeatParam)
        {
            int result = rn.Next(max);
            if (result == notRepeatParam)
            {
                result++;
            }
            if (result > max)
